...
== Description of the target enviroment
...
* Disk Size: 8GB
* AMI Image: Amazon Linux 2 AMI (HVM), SSD Volume Type
* Network Acess: SSH from any source, mySQL from web servers
* Tag: Managed:Manual

=== Additional steps: 

To run the web appliction some packages and configuration are need. 

* Additional Software: git, rubb, mariadb developer libraries, build essentials and policykit-1
* gem package: bundler
* create the folder "/opt/iac_shop" and do clone the git repoitory *{git}*
...
=== Access the Amazon Webservices Console and create your user to work with
...
=== Create a VPC
The VPC is your virtual datacenter, where all the ressources will be located. 
The VPC boudary is the network. 
