...
And the follwoing tasks should have been completed:
...
* The Github repo
** *Project*: iacstart
* Choos the Playbook from the repo
...
You could now run the Workflow (it would of course only execute the first Playbook) but something is missing: Have a look at the `create_vpc_complete.yml` Playbook in Github. In the Ansible-code are a good number of *{{ ... }}* constructs. These are variables which have to be filled with content, making the Playbook more versatile without always changing the code itself.

There are multiple ways to supply variable content to a Playbook/Job Template, in out case put it into the Worflow definition in Tower:
...
* AMI IDs are region-specific, on the next page choose *EU (Frankfurt)* as *Region* and, lo and behold, you'll get the AMI ID to the right.
* Copy it
...
* Define the variables needed by the Playbook (fill in the AMI ID you found for the CentOS AMI and the name of your SSH key) by adding the following to the *EXTRA VARIABLES*:
